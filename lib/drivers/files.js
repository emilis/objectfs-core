...
/*
    Module for storing objects in files.
*/
 ...
 *
 */
exports.connect = function(uri, io) {

    this.file_dir = io.getDirectory(uri);
    this.extension = (uri.params && uri.params.extension) ? uri.params.extension : this.extension;
};
...
exports.getIdFromFileName = function(file_name) {
    ...
    return file_name.replace(this.file_dir, "").slice(0, 0-this.extension.length);
};
...
exports.iterate = function(path, options) {
    ...
    for each (var fpath in gen) {
        if (pattern === undefined || fpath.match(pattern)) {
            ...
            doc._id = this.getIdFromFileName(fpath);
            yield doc;
        }
        if (limit !== undefined) {
            if (!limit) {
 ...
 * event is one of: onRead, onWrite, onCreate, onUpdate, onRemove.
 * handler is a function callback that takes three arguments: event name, object id, object.
 */
